function generateTOC() {
    ...
    function onScroll() {
        ...
        for (let i = headingSections.length - 1; i >= 0; i--) {
            if (scrollPosition >= headingSections[i].offsetTop) {
        ...
        for (const link of menuLinks) {
            link.classList.toggle('active', link.getAttribute('href') === currentFile + '#' + currentId);
        }
    }
